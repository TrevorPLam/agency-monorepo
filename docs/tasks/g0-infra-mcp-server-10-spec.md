...
export function registerPackageTools(server: McpServer, repoRoot: string): void {
  ...
  server.tool(
    'read_package_readme',
    'Read the 01-config-biome-migration-50-ref-quickstart.md for a specific package',
    {
      packageName: z.string().describe('Package directory name'),
      directory: z.enum(['apps', 'packages', 'tools']).default('packages'),
    },
    async ({ packageName, directory }) => {
      const readmePath = path.join(repoRoot, directory, packageName, '01-config-biome-migration-50-ref-quickstart.md');
      try {
        ...
        return {
          content: [{
            type: 'text',
            text: `01-config-biome-migration-50-ref-quickstart.md not found for ${directory}/${packageName}`,
          }],
          isError: true,
        };
      }
...
async function getPackageDetails(repoRoot: string, directory: string, packageName: string) {
  ...
  try {
    ...
    return {
      ...
      fileCount,
      hasReadme: await fileExists(path.join(packagePath, '01-config-biome-migration-50-ref-quickstart.md')),
